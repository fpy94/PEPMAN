...
class Net(nn.Module):
	def __init__(self):
		...
		self.prelu1=nn.PReLU(128*201)
		self.drop1=nn.Dropout(0.25)
		self.conv2=nn.Sequential(
			nn.Conv1d(128,128,7,padding=3),
			nn.BatchNorm1d(128,momentum=0.01)
		)
		self.prelu2=nn.PReLU(128*201)
	...
	def forward(self,x):
		...
		self.conv1_x=feature_x.view(feature_x.size(0),128,201)
		...
		self.conv2_x=feature_x.view(feature_x.size(0),128,201)

		feature_x=self.conv2_x.permute(0,2,1)
		self.att_x=self.head_att(feature_x)#N,201,1
		x=feature_x.permute(0,2,1).bmm(self.att_x)#N,128,1
		x=x.view(x.size(0),-1)
		x=self.MLP(x)
		return x
